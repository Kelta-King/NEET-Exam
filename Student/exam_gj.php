...
if(isset($_SESSION['login_student']) && isset($_REQUEST['dtatm']) && isset($_SESSION['exam_subject']) && isset($_SESSION['exam_start']) && isset($_SESSION['student_lang'])){
    ...
    $qry = "SELECT q_number, q_data_guj, q_op1_guj, q_op2_guj, q_op3_guj, q_op4_guj FROM questions WHERE q_number = $q_number AND q_sub = '".$_SESSION['exam_subject']."'";

    if($dt = $conn->query($qry)){
        
        $rlt = $dt->fetch_assoc();
        $q_number = $rlt['q_number'];
        $q_data = $rlt['q_data_guj'];
        $op1 = $rlt['q_op1_guj'];
        $op2 = $rlt['q_op2_guj'];
        $op3 = $rlt['q_op3_guj'];
        $op4 = $rlt['q_op4_guj'];
...
<script src="JS/exam_gj.js"></script>
</body>
</html>
<?php
    $conn->close();
